...
def cusum_detector(
    ...
    if sigma <= 0:
        raise ValueError("The observations must have non-zero variance.")

    standardized = (values - baseline) / sigma
    ...
    for i, value in enumerate(standardized):
        ...
        previous_positive = positive[i - 1] if i > 0 else 0.0

        previous_negative = negative[i - 1] if i > 0 else 0.0

        positive[i] = max(
            0.0,
            previous_positive + value - k,
        )

        negative[i] = min(
            0.0,
            previous_negative + value + k,
        )

        # Match the companion repository:
        # threshold crossing does NOT reset the statistics.
        if positive[i] > h or abs(negative[i]) > h:
            alarms[i] = 1
...
def run_bocd(
    ...
    return pd.DataFrame(
        {
            "bocd_expected_run_length": expected_run_length(R),
            "bocd_map_run_length": map_run_length(R),
            # Retained for completeness.
            # With constant hazard this is not a
            # data-driven change-point score.
            "bocd_run_length_0": R[1:, 0],
        }
    )
...
def run_comparison(
    ...
    comparison["log_cases"] = log_cases

    comparison["cusum_positive"] = cusum_results["cusum_positive"]

    comparison["cusum_negative"] = cusum_results["cusum_negative"]

    comparison["cusum_alarm"] = cusum_results["cusum_alarm"]

    comparison["bocd_expected_run_length"] = bocd_results["bocd_expected_run_length"]

    comparison["bocd_map_run_length"] = bocd_results["bocd_map_run_length"]

    comparison["bocd_run_length_0"] = bocd_results["bocd_run_length_0"]
...
if __name__ == "__main__":
    ...
    print("CUSUM parameters: " "k=0.3, h=1.5")

    print("CUSUM baseline: median")

    print("CUSUM reset after alarm: no")

    print("BOCD prior expected " "run length: lambda=6.0\n")
    ...
    print(alarm_years if alarm_years else "None")

    print("\nYears with the smallest " "BOCD expected run length:")
    ...
    print(
        ...
        .sort_values("bocd_expected_run_length")
        .head(5)
