...
def load_measles_data(
    ...
    df = df[["year", "cases"]].copy().sort_values("year").reset_index(drop=True)

    if df.empty:
        raise ValueError("The measles data file is empty.")
    ...
    if not np.all(np.isfinite(cases)):
        raise ValueError("Case counts contain non-finite values.")

    if np.any(cases <= 0):
        raise ValueError(
            "All case counts must be positive because " "the analysis uses log(cases)."
        )
...
def preprocess_counts(
    ...
    if cases.ndim != 1:
        raise ValueError("cases must be one-dimensional.")

    if len(cases) == 0:
        raise ValueError("cases cannot be empty.")

    if not np.all(np.isfinite(cases)):
        raise ValueError("cases contains non-finite values.")

    if np.any(cases <= 0):
        raise ValueError("All case counts must be positive.")
...
class NormalGammaPredictive:
    ...
    def update(
        ...
        new_beta = self.beta + (self.kappa * (x - self.mu) ** 2) / (
            2.0 * (self.kappa + 1.0)
        )
...
def constant_hazard(
    ...
    if lam <= 1.0:
        raise ValueError("lam must be greater than 1.")
...
def bocd(
    ...
    for t in range(
        ...
        log_pred = model.log_pred_prob(x)
        ...
        log_survival = np.log1p(-hazard)

        # Existing run continues.
        log_growth = log_R_previous + log_pred + log_survival

        # New run begins.
        log_cp = logsumexp(log_R_previous + log_pred + log_hazard)
        ...
        log_R_current -= logsumexp(log_R_current)

        R[t, : t + 1] = np.exp(log_R_current)

        model.update(x)

        log_R_previous = log_R_current
...
def expected_run_length(
    ...
    run_lengths = np.arange(R.shape[1])

    return (R[1:] * run_lengths).sum(axis=1)
    ...
    return pd.DataFrame(
        {
            ...
            "log_cases": log_cases,
            "expected_run_length": expected_run_length(R),
            "map_run_length": map_run_length(R),
            "posterior_run_length_0": changepoint_probability(R),
        }
    )
